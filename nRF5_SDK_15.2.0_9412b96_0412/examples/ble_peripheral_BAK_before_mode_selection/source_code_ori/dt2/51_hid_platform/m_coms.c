...
//wright : this header is not used in prog
...
static void m_coms_regret_write(protocol_mode_t protocol)
{
     ...
     {   
			 //wright : fix errors cause by dif SD version 
        //err_code = sd_power_gpregret_clr(0x0000000E);
				err_code = sd_power_gpregret_clr(0,0x0000000E);
			 
         APP_ERROR_CHECK(err_code);
			 
			 //wright : fix errors cause by dif SD version 
        //err_code = sd_power_gpregret_set(((uint32_t)protocol) << 1);
				err_code = sd_power_gpregret_set( 0,((uint32_t)protocol) << 1);
...
static protocol_mode_t m_coms_regret_read(void)
{
    ...
    {   
        ...
        //err_code = sd_power_gpregret_get(&regret);
				err_code = sd_power_gpregret_get(0,&regret);
...
static void m_coms_switch_protocol_handler(void * p_event_data, uint16_t event_size)
{
    ...
    {
        case protocol_mode_ble:
						//wright : fix for fit 52 series method usage
...
//wright : merge 52 series ble evt handler 
static void m_coms_ble_evt_handler(void * p_event_data, uint16_t event_size)
{
    ...
    APP_ERROR_CHECK_BOOL(event_size == sizeof(m_coms_ble_evt_t));
		...
		//wright : debug print is not needed.
    ...
    switch (p_evt->type)
    {
        ...
        case M_COMS_BLE_EVT_DATA_RECEIVED:
					//wright : fix it according to 52 series data struct
...
//            evt.data.data_received.report_idx    = p_evt->data.data_received.report_idx;
						evt.data.data_received.data          = p_evt->data_received.data;
            evt.data.data_received.len           = p_evt->data_received.len;
            evt.data.data_received.interface_idx = p_evt->data_received.interface_idx;
            evt.data.data_received.report_type   = p_evt->data_received.report_type;
            evt.data.data_received.report_idx    = p_evt->data_received.report_idx;
            break;
        
				
				//wright : not used in dt2
...
//        
        case M_COMS_BLE_EVT_ADV_TIMEOUT:
            ++m_coms_timeouts;
            if ((m_coms_timeouts == M_COMS_ADV_TIMEOUTS) || (m_coms_protocol_scheme != protocol_mode_auto) || (m_coms_ble_adv_bond_stored()))
            {
                if (m_coms_bonding_state == M_COMS_BONDING_STATE_IDLE)
                {
...
//            break;
        ...
        case M_COMS_BLE_EVT_DISABLED:
            ...
            break;
				...
				/*wright : Merge 52 series ble events*/
						
				 case M_COMS_BLE_EVT_ADV_IDLE:
						//err_code = bsp_indication_set(BSP_INDICATE_IDLE);
						APP_ERROR_CHECK(err_code);
						//wright : use the 52 series sleep methods
        ...
        /*wright : Merge 52 series ble events -- end*/

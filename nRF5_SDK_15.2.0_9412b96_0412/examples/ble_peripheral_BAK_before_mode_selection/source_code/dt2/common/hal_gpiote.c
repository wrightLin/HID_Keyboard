...
//wright : defined in nrfx_gpiote as "nrfx_gpiote_irq_handler"
...
#error "Please define either SOFTDEVICE_PRESENT or SOFTDEVICE_NOT_PRESENT to state if a Softdevice is present or not."
    ...
    {
			//wright
        sd_nvic_SetPriority(GPIOTE_IRQn, 3);//NRF_APP_PRIORITY_LOW);
    ...
    {
			//wright
        NVIC_SetPriority(GPIOTE_IRQn,3 );//NRF_APP_PRIORITY_LOW);

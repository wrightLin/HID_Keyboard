...
//JS Modify: 2/18/2019: for debug only
//#include "nrf_log.h"
...
static bool                      m_coms_gzll_encrypted           = false;
static bool                      m_coms_gzll_tx_attempted        = false;
static uint16_t                  m_coms_gzll_max_tx_attempts     = 0;
...
//				NRF_LOG_INFO("m_coms_gzll_tx_attempted = %d", m_coms_gzll_tx_attempted);
//				if(!m_coms_gzll_tx_attempted)
//				{
//          nrf_gzll_error_code_t gzll_err = nrf_gzll_get_error_code();
//					NRF_LOG_INFO("gzll_err_code = %d", (uint8_t) gzll_err);
//        }					
    }        
    else
    {
...
uint32_t m_coms_gzll_init(const app_sched_event_handler_t event_callback)
{
    ...
    m_coms_gzll_max_tx_attempts     = s_gzll_params.tx_attempts;
    m_coms_gzll_encrypted           = s_gzll_params.encrypt;
    m_coms_gzll_timeslot_period     = s_gzll_params.timeslot_period;
    m_coms_gzll_sync_lifetime       = s_gzll_params.sync_lifetime;
    m_coms_gzll_tx_attempted        = false;
    m_coms_gzll_have_system_address = false;
    m_coms_gzll_have_host_id        = false;    
...
uint32_t m_coms_gzll_enable(bool erase_bonds)
{
    ...
    nrf_gzll_set_max_tx_attempts(m_coms_gzll_max_tx_attempts);
...
{	
    ...
    if (!m_coms_gzll_have_system_address)
    {
			m_coms_gzll_send_event(gzll_event_search_for_host);   //JS Modify: 1/23/2019   
    ...
    if (m_coms_gzll_have_system_address)
    {
        ...
        {         
				...
				//JS Modify: 1/25/2019  Send host id request if encrypted enable
				if(m_coms_gzll_encrypted)        
        {
            if(!m_coms_gzll_have_host_id)
            {

...
//wright : get current mode for module
//#include "m_coms.h"
...
//wright : .\_build\nrf52810_xxaa.axf: Error: L6200E: Symbol POWER_CLOCK_IRQHandler multiply defined (by nrfx_clock.o and m_pwr_and_clk_mgmt.o).
...
uint32_t m_pwr_and_clk_mgmt_init(const m_prw_and_clk_mgmt_init_t* p_params, app_sched_event_handler_t event_handler)
{
    ...
    hfclk_crystal_oscillator_start(true);    
    //wright
    return app_timer_start(m_pwr_mgmt_timer_id, APP_TIMER_TICKS(PWR_MGMT_POLLRATE), 0); //APP_TIMER_PRESCALER), 0);
}
...
void m_pwr_mgmt_run(void)
{
    ...
    #else
				//wright : get current mode 
        switch (get_current_mode())//switch (m_coms_protocol_mode_get())
        {
            case M_COMS_BLE_MODE: // case protocol_mode_ble:
                err_code = sd_app_evt_wait();
                if (err_code == NRF_ERROR_SOFTDEVICE_NOT_ENABLED)
                {
            ...
            case M_COMS_GZLL_MODE: //case protocol_mode_gzll:
                __WFE();
